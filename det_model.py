...
# Remove rows where region_shape_attributes is {} or values are 0
...
def get_datasets(df, image_dir, transform, train_frac=0.99):
    df_sample = df.sample(1980)
...
def main(df, image_dir, num_epochs=10, train_frac=0.99, batch_size=10, lr=0.005):
    # Set the random seed for reproducibility
    random.seed(42)
    torch.manual_seed(42)

    transform = transforms.Compose([transforms.ToTensor()])
    train_dataset, val_dataset = get_datasets(df, image_dir, transform, train_frac)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            random_indices = random.sample(range(len(val_dataset)), min(len(val_dataset), 10))
            for idx, data_idx in enumerate(random_indices):
                img, target = val_dataset[data_idx]
                img = img.unsqueeze(0).to(device)
                pred = model(img)
                visualize_prediction(
                    img=img[0].cpu(),
                    target=pred[0],
                    epoch=epoch + 1,  # epoch number, starting from 1
                    img_idx=idx  # index in the current set of visualizations
                )

    model_save_path = "C:\\Users\\emili\\OneDrive\\Escritorio\\BLOODSMEARS\\det_20model.pth"
