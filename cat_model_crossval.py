...
def save_model(model, filepath=r"C:\Users\emili\OneDrive\Escritorio\BLOODSMEARS\cat_model3"):
    """
...
def main():
    ...
    np.random.shuffle(indices)  # Shuffle the indices before splitting to ensure random distribution
    ...
    for fold in range(K_FOLD):
        ...
        train_indices = indices[:val_start_idx] + indices[val_end_idx:]
        
        # Create SubsetRandomSampler with indices for train and validation
        train_subsampler = torch.utils.data.Subset(full_dataset, train_indices)
        val_subsampler = torch.utils.data.Subset(full_dataset, val_indices)
        val_subsampler.dataset.transform = test_transform  # apply the test transformations for validation data
        print(len(val_subsampler))
        print(len(train_subsampler))
        history, model = run_single_fold(train_subsampler, val_subsampler, full_dataset_classes)
        histories.append(history)

        # Save the model after training this fold
        save_path = r"C:\Users\emili\OneDrive\Escritorio\BLOODSMEARS\cat_model_elchlo_fold_{}".format(fold+1)
        save_model(model, filepath=save_path)
        
    # After training all folds, compute the average accuracy and loss for visualization
    avg_train_loss = np.mean([h['train_loss'] for h in histories])
    avg_val_loss = np.mean([h['val_loss'] for h in histories])
    avg_train_acc = np.mean([h['train_acc'] for h in histories])
    avg_val_acc = np.mean([h['val_acc'] for h in histories])
    
    print(f"Average Train Loss: {avg_train_loss:.4f}, Average Train Accuracy: {avg_train_acc:.4f}")
